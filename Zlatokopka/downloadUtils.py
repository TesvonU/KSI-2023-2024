...
def downloadUrl(url, root, filename):
    """
...
def extractArchive(from_path, to_path=None, remove_finished=False):
    """
...
def downloadDataset(url, data_dir, dataset_zip_name, force_download=False):
    """
    Download dataset
    ...
    if not os.path.exists(data_dir) or not os.listdir(data_dir) or force_download:
        ...
        data_file = downloadUrl(url, data_dir, dataset_zip_name)
        extractArchive(data_file, remove_finished=True)
